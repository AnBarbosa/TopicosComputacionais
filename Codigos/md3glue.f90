! MD3glue: a simple, minimal molecular dynamics program in Fortran 90
!
! Same as MD3, but uses an ab-initio derived glue many-body potential 
! for Aluminum, instead of a Lennard-Jones.
! Now units are:
!    Distances in A, energies in eV, temperatures in K.
! Equilibrium structure at T=0: fcc, a=4.032 A, E=-3.36 eV/atom 
!
! It must be linked with the supplied file 'aluminum.f'.
!
...
!     when the number of atoms N is known.
   ...
   double precision, dimension(:), allocatable :: coord     ! coordinations
   double precision, dimension(:), allocatable :: deru      ! U'(n_i)
...
!
   ...
   double precision, parameter :: k_B = 8.617385d-5  ! Boltzmann constant (1986)
end module Simulation_Control

module Potential
!     
!     Contains parameters and tables of the ab-initio derived glue potential
!     for Aluminum [F. Ercolessi and James B. Adams, Europhysics Letters
!     26, 583 (1994) ].
!     The tables are filled by calling externally defined subroutines
!     v2, rh, uu, returning the actual potential.
!
   double precision, parameter :: Rcutoff = 0.555805441821810D+01 ! cutoff
   integer, parameter :: TableSize = 20001
   double precision, parameter :: Rmin = 0.055805441821810D+01 ! cutoff-5 A
   ...
   double precision, parameter :: CoordMax = 1.2d0
   double precision, parameter :: DeltaCoord = CoordMax / ( TableSize - 1 )
   double precision, parameter :: InvDeltaCoord = 1.d0 / DeltaCoord
   double precision, dimension(TableSize) :: PhiTab, DPhiTab !phi(r),1/r dphi/dr
   double precision, dimension(TableSize) :: RhoTab, DRhoTab !rho(r),1/r drho/dr
   double precision, dimension(TableSize) :: UTab,DUTab      !U(n), du/dn
...
TRequested = k_B * TRequested              ! user sees K, we use eV internally
...
if (ConstantT) then
   print '(a,f12.2)','# Constant T run with T =',TRequested/k_B
else
...
!
!  Compute (once for all) tables for the glue potential for Al
!
use Potential
implicit none
double precision :: Rsq,r,phi,dphi,d2phi,rho,drho,d2rho,c,glue,dglue,d2glue
integer :: k
do k=1,TableSize
...
double precision :: Rsqij,rho,drho,phi,dphi,rk,weight
integer :: i,j,k,L
!
...
print '(a,/,a,f14.2,4f14.6)','#','# Means', &
                     temperature_sum / k_B / Nsteps , &
                     ene_kin_sum / Nsteps , &
                     ene_pot_sum / Nsteps , &
